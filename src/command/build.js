...
module.exports = sc('build', function (program, done) {
    ...
    async.eachSeries(pkg.transform, function (transformer, next) {
        getPlugin('transform', transformer)(program).transform.call(null, clone, model, function (err, result) {
            model.code = result;
            next(err);
        });
    }, transport);
    ...
    function transport () {
        ...
        async.each(pkg.transport, function (transporter, next) {
            getPlugin('transport', transporter)(program).transport.call(null, clone, model, next);
        }, done);
